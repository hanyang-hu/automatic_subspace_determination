...
@torch.no_grad()
def visualize_results(output_dir: Path, model_name: str, model, likelihood, train_x, train_y, test_x, test_y, pred_mean, losses):
    ...
    if test_x.shape[-1] >= 2:
        ...
        likelihood.eval()
        with gpytorch.settings.fast_pred_var():
            pred_grid = likelihood(model(x_grid)).mean.reshape(grid_n, grid_n).cpu().numpy()
